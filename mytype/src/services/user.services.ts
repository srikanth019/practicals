...
export class UserService {
	public async signup(userData: User, avatar: string | undefined): Promise<User> {
		...
		if (existingUser) {
			...
			throw new ApiError(409, `This ${field} ${userData[field]} already exists`);
		}
	...
	public async login(userData: User): Promise<{ accessToken: string; refreshToken: string }> {
		...
		// type assertion
		const { accessToken, refreshToken } = (await generateAccessAndRefreshToken(user._id)) as { accessToken: string; refreshToken: string };
	...
	public async getNewRefreshToken(refreshToken: string): Promise<{ accessToken: string; refreshToken: string }> {
	...
	public async changePassword(user: UserDocument, currentPassword: string, newPassword: string) {
	...
	public async GetUsers(page: number, limit: number): Promise<{ users: any[]; userCount: number | string }> {
	...
	public async UpdateUser(userId: string | Types.ObjectId, fullName: string, username: string, avatar: string | undefined) {
		const existingUser = await UserModel.findById(userId);
		if (!existingUser) {
			throw new ApiError(404, 'User not found');
		}

		const avatarUrl = avatar ? await uploadCloudinary(avatar) : existingUser.avatar;
...
const generateAccessAndRefreshToken = async (userId: string | Types.ObjectId) => {
	try {
		...
		throw new ApiError(500, 'Something went wrong while generating generating access and refresh token');
	}
};
